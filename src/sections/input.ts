 ...
 * mode, character counter, and the one fetch in the product. Examples replay
 * a saved, gate-checked result instantly; live searches are always explicit.
 *
...
export function initInput(): void {
  ...
  function loadSample(id: string): void {
    ...
    if (recorded) renderRecorded(recorded);
  }
  ...
  function renderRecorded(recorded: RecordedResult, reason?: string): void {
    ...
    if (reason) {
      setStatus(`${reason} Showing the saved example result instead.`, true);
    } else {
      setStatus("");
    }
    renderResults(kept, {
